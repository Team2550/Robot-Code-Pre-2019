...
Robot::Robot() :
		driveController(0), perifController(1), cameraTracking(400, 300, 10), ultrasonic(
				0, (5 / 4.88) * (1000 / 25.4), 1), // (5 mm / 4.88 mV) * (1/25.4 in/mm) * (1000 mV/V)
		bumperSwitch(0), driveBase(0, 1) {
...
void Robot::RobotInit()
{
	...
	cameraTracking.LaunchVisionThread();
}
...
void Robot::TeleopPeriodic() {
	float leftSpeed = Utility::Deadzone(
			-driveController.GetRawAxis(axisTankLeft));
	float rightSpeed = Utility::Deadzone(
			-driveController.GetRawAxis(axisTankRight));
